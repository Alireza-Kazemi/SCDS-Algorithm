...
nFFT = 512;
Fnum=62;
load ReadyDataRBD.mat % Load Data here; Data should be in the format of cell array on participants as {[Epochs, Samples, Channels]}
...
for Participant = 1:length(DataEEG)
    ...
    for Ch=1:ChanNum        
        ...
        for ix=1:Xnum
            ...
            desiredFreqs = [1,3,5,7,9,11,13,15,20,28];
			[~,IndsF] = findpeaks(-min(abs(repmat(FFF,length(desiredFreqs),1)-repmat(desiredFreqs',1,length(FFF)))));
            CH_feats(ix,35:44) = abs(BTS(IndsF));
            CH_feats(ix,45:54) = angle(BTS(IndsF));
            
            CH_BTS = cat(1,CH_BTS,BTS);
