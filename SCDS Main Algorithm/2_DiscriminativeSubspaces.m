...
%% Find significantly different centers by a relative comparison
...
FeatIDX1 = cell(1,Cnum);
FeatIDX08 = cell(1,Cnum);
FeatIDXleast  = cell(1,Cnum);
Inds = 1:Fnum;
for i=1:Cnum
    DD = Diffs(:,i);
    FeatIDX1{i}= Inds(DD>=.1);
    FeatIDX08{i}= Inds(DD>=.08);
...
for i=1:5
    disp(Labels{i});
    disp((CH_Lab(FeatIDX08{i})'));
end

Centers_FPz = Centers;
FeatIDX1_FPz = FeatIDX1;
FeatIDX08_FPz = FeatIDX08;
...
for i=1:Cnum    
    subplot(Cnum,1,i)
    plot(Diffs(IDX{i},i),'Color',ColorCodes(i,:),'LineWidth',1);
    hold on
    plot(Diffs(IDX{i},i),'*','Color',ColorCodes(i,:));
    xticks(1:Fnum)
    xticklabels(num2str(IDX{i}(1:Fnum)))
    title([Labels{i},' vs other Stages in Pz-Oz'])
    ylabel('Difference')
    xtickangle(90)
    plot(0:62,repmat(.08,1,63),'r--')
%     grid on
axis tight
end
xlabel('Feature Index')
...
Centers_Pz = Centers;
FeatIDX1_Pz = FeatIDX1;
FeatIDX08_Pz = FeatIDX08;
FeatIDXleast_Pz = FeatIDXleast;
%% Save
FeatIDX1 = cat(1,FeatIDX1_FPz,FeatIDX1_Pz);
FeatIDX08 = cat(1,FeatIDX08_FPz,FeatIDX08_Pz);
FeatIDXleast = cat(1,FeatIDXleast_FPz,FeatIDXleast_Pz);
Centers = cat(1,Centers_FPz,Centers_Pz);


x = 1; ACC(1,x) = Acc(length(FeatIDX08_Pz{x}),x);
x = 2; ACC(1,x) = Acc(length(FeatIDX08_Pz{x}),x);
x = 3; ACC(1,x) = Acc(length(FeatIDX08_Pz{x}),x);
x = 4; ACC(1,x) = Acc(length(FeatIDX08_Pz{x}),x);
x = 5; ACC(1,x) = Acc(length(FeatIDX08_Pz{x}),x);
x = 5; ACC(2,x) = Acc(length(FeatIDX08_FPz{x}),x);
x = 4; ACC(2,x) = Acc(length(FeatIDX08_FPz{x}),x);
x = 3; ACC(2,x) = Acc(length(FeatIDX08_FPz{x}),x);
x = 2; ACC(2,x) = Acc(length(FeatIDX08_FPz{x}),x);
x = 1; ACC(2,x) = Acc(length(FeatIDX08_FPz{x}),x);

save('FeatIDXnew.mat','FBest','IDX','FeatIDX1','FeatIDX08','Centers','FeatIDXleast')
...
Shapes = {'^','s','d','p','h'};
for i=1:Cnum
    Diffs(:,i) = Centers{Ch_idx,i}(:,1)-Centers{Ch_idx,i}(:,2);
    plot((1:Fnum)',Diffs(:,i),['-',Shapes{i}]);
end
grid on;
xlabel('Feature index')
ylabel('Mean difference on FPz-Cz')
xlim([0,63])
...
grid on;
xlabel('Feature index')
ylabel('Mean difference on Pz-Oz')
xlim([0,63])
legend(Labels,'Location','west')
box on
